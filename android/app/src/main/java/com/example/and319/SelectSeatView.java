...
//                LogUtil.i(currentX + "--before--" + currentY);
    ...
    private void drawOverBorder(Canvas canvas) {
        //绘制移动的框，应该显示屏幕区域内的row位,屏幕点转换到画布上，在转换到缩略图上
...
//            float averageColumn = overRectWidth / seatList[i].length;
            for (int x = 0; x < seatList[i].length; x++) {
                ...
                int seatState = seatList[i][x];         //seatstate 为属性是否已售
                ...
                //Record rectangular position information
    ...
    private void drawSeatView(Canvas canvas) {
        ...
        //How many rows of seats are drawn
        for (int i = 0; i < seatList.length; i++) {
            int top;
            if (i == 0) {
...
//                LogUtil.i(left + "--" + top + "--" + (left + seatWidth) + "--" + (top + seatWidth));
                ...
                //Need to calculate seats drawn from the middle
                switch (seatState) {
                    case EMPTY_SEAT:
                        emptyCount++;
                        paintSeat.setColor(Color.TRANSPARENT);
                ...
                if (isSelect) {
                    paintSeat.setColor(Color.GREEN);
                    canvas.drawRect(rect, paintSeat);
                    canvas.drawText("line"+selectRectBean.getRow()  + " seat"+selectRectBean.getColumn(), rect.left, rect.top + seatWidth / 2, textPaint);
                }
                //收集所有的位置信息
...
//                paintSeat.setColor(Color.GREEN);
//                canvas.drawRect(rect, paintSeat);
//                canvas.drawText(selectRectBean.getRow() + "line" + selectRectBean.getColumn() + "列", rect.left, rect.top + seatWidth / 2, textPaint);
//            }
//        }
    ...
    private void drawFilmScreen(Canvas canvas) {
        ...
        path1.close();
        canvas.drawPath(path1, screenPaint);
        canvas.drawText("Screen", (centerX - filmScreenHeight / 4), transformOldCoordY(filmScreenHeight / 4), textPaint);
